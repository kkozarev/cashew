pro aia_check_dirs,basepath,stt,ett,report=report
;PURPOSE:
...
;check for year
  if not dir_exist(basepath+st[0]) then begin
     report+='No data for year '+st[0]+'; '
     exec='mkdir -m 775 '+basepath+st[0]
     spawn,exec
  endif

;check for month
  if not dir_exist(basepath+st[0]+'/'+st[1]) then begin
     report+='No data for month '+st[1]+'; '
     exec='mkdir -m 775 '+basepath+st[0]+'/'+st[1]
     spawn,exec
  endif

;check for day
  if not dir_exist(basepath+st[0]+'/'+st[1]+'/'+st[2]) then begin
     report+='No data for day '+st[2]+'; '
     exec='mkdir -m 775 '+basepath+st[0]+'/'+st[1]+'/'+st[2]
     spawn,exec
  endif

;check for hours
  if not dir_exist(basepath+st[0]+'/'+st[1]+'/'+st[2]+'/H'+st[3]+'00') then begin
     report+='No data for hour '+st[3]+'; '
     exec='mkdir -m 775 '+basepath+st[0]+'/'+st[1]+'/'+st[2]+'/H'+st[3]+'00'
     spawn,exec 
  endif
  ...
  if st[3] ne et[3] then begin
     for t=fix(st[3]),fix(et[3]) do begin
        ...
        if not dir_exist(basepath+st[0]+'/'+st[1]+'/'+st[2]+'/H'+hr+'00') then begin
           exec='mkdir -m 775 '+basepath+st[0]+'/'+st[1]+'/'+st[2]+'/H'+hr+'00'
           spawn,exec
        endif
     endfor
  endif

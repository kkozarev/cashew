pro test_aia_update_source_position_manual
  ...
  event=load_events_info(label='131119_01')
  aia_inspect_source_position_manual, event
end


pro aia_inspect_source_position_manual, event
  !p.multi=0
  !p.background=255
  !p.color=0
...
;Overplot the lateral measurements location
  ...
  if newarlon ne get_polar_angle(event) then begin
     ...
     if uinput eq 'y' then begin
        ...
        coordx=fix(tmp[0])
        coordy=fix(tmp[1])
        print,event.coordx,event.coordy
        print,coordx,coordy
        print,event.arlon,newarlat
        print,event.arlon,event.arlat
        ...
        print,'Saved updated JSON events file.'
     endif
  endif

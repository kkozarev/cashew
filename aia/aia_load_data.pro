...
;Run for one or a few events like this:
  ...
  if one eq 1 then begin
     ...
     for ev=0,n_elements(labels)-1 do begin
        ...
        for w=0,n_elements(wavelengths)-1 do begin
           ...
           aia_load_data,event.st,event.et,wav,event=event,/subroi,/force,/remove_aec
        endfor
     endfor
  endif
  ...
  if all eq 1 then begin
     ...
     for ev=0,n_elements(events)-1 do begin
        ...
        for w=0,n_elements(wavelengths)-1 do begin
           wav=wavelengths[w]
           aia_load_data,event.st,event.et,wav,event=event,subdata=subdata,$
                         subindex=subindex,/subroi,/force,/remove_aec
        endfor
     endfor
  endif
...
;      aia_inspect_map,map,submap=submap
;   endelse
  if keyword_set(event) then aia_archive_local_data,event

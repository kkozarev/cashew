...
function sun_ra_dec,JD,R=R,B0=B0,alpha=alpha,delta=delta,lambda=lambda,alt_r=r_alt
;+
...
;
; Adapted from Solarsoft's eve_sun_almanac.pro  
  ...
  earth_sun_unit_vector=[[cos(alpha*ddtor)*cos(delta*ddtor)],[sin(alpha*ddtor)*cos(delta*ddtor)],[sin(delta*ddtor)]];
  ...
  for i=0,n_elements(B0)-1 do begin
     B0[I]=-asin(total(nG*earth_sun_unit_vector[I,*]))*dradeg ;use dot product of earth-sun vector and sun north pole to get angle. Use asin to get 0deg when vectors are perpendicular
  endfor
  result=[alpha,delta]
  return,result
end

...
;
  ...
  savepath=event.mfhcpath
                                ;infile=find_latest_file(event.mfhcpath+'csgs_results_*')
  tIIfilename='vasili_out.txt'
  infile=file_search(event.mfhcpath+'csgs_results_'+event.date+'_'+event.label+'_lores.sav')
  if keyword_set(hires) then infile=file_search(event.mfhcpath+'csgs_results_'+event.date+'_'+event.label+'_hires.sav')
  ...
  restore,infile
                                ...
                                ;TEMPORARY!!!
  startstep=0
  crosspoints=crosspoints[startstep:*,*]
  csgsradius=csgsradius[startstep:*]
  radiusfitlines=radiusfitlines[startstep:*]
  subindex=subindex[startstep:*]
  time=time[startstep:*]
                                ;TEMPORARY!!!
...
;+======================================================================
  ...
  if file_test(fname) and not keyword_set(force) then begin
     ...
     PLOT, tm, allcrosses, PSYM = 10, $ 
           TITLE = thlet+'!DBN!N Evolution, '+event.date, $
           XTITLE =xtit, $
           YTITLE = ytit, $

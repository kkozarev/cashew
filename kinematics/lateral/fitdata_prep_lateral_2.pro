pro fitdata_prep_lateral_2,lat_data,plotinfo
...
;ADDITIONAL PROCESSING WITH HISTOGRAM SELECTION AND ATROUS sharpening
  ...
  !p.position=position
  aia_plot_jmap_data,time.jd,lat_data.x_deg_array[yrng[0]:yrng[1]],$
...
;Average out each lateral distance/row of the image.
...
;------------------------------  
  ...
  diffdata_atrous=cashew_atrous_multiscale_one_level(lat_data.data,coeffs=coefficients,/denoise)
  
  ;wdef,3,1000,800
  position=[0.09,0.11,0.48,0.5]
  !p.position=position
;atrous(histogram(X-smoothed data))
  aia_plot_jmap_data,time.jd,lat_data.x_deg_array[yrng[0]:yrng[1]],$
...
;------------------------------     
  ...
  !p.position=position
  histdata=current_data
  sm=histdata[*,yrng[0]:yrng[1]]-smooth(histdata,[1,14],/edge_truncate)
  sm_histdata=histdata-sm
;histogram(atrous(histogram(X-smoothed data))) - y-smooth
  aia_plot_jmap_data,time.jd,lat_data.x_deg_array[yrng[0]:yrng[1]],$
                     sm_histdata[*,yrng[0]:yrng[1]],$
                     min=-20,max=50,title='',xtitle=plotinfo.xtitle,$
                     ytitle=plotinfo.ytitle,position=position,charsize=2
  current_data=histdata-sm
;------------------------------
  ...
  endwhile
  case uinput of
     '1': lat_data.data=diffdata
     '2': lat_data.data=xsmdata
     '3': lat_data.data=xsmdata_atrous
     '4': lat_data.data=sm_histdata
  endcase
  
  ;bin_aia_data_lateral,lat_data,binData=binData
  ;make_glatient_map_lateral, lat_data, binData, intensityData=intensityData
  ;mask_glatient_data_lateral, lat_data, intensityData, correctData = correctData
  ;stop
;----------------------------  
wait,1
wdel,0

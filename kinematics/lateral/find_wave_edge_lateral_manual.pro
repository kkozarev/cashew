...
pro find_wave_edge_lateral_manual, event, lat_data, plotinfo, exit_status
;PURPOSE
;Procedure to find the edges of the EUV wave using a Gaussian fit
;At each timestep a Gaussian is fitted to the lateral data and the
...
;                     the wave
  ...
  if lat_data.type eq 'lateral_left' then last_saved_start_end=event.annuluspath+'lateral_left_wave_extent.last'
  if lat_data.type eq 'lateral_right' then last_saved_start_end=event.annuluspath+'lateral_right_wave_extent.last'
  ...
  ny=n_elements(lat_data.data[0,*])
  data_background=mean(lat_data.data[0:9,ny-25:ny-1])
...
; Filter out the negative data
        ...
        plot, lat, col, xtitle='Lateral distance, deg',ytitle='Pixel intensity',charthick=2,/ystyle,$
              title=strtrim(string(ii),2)+' Wave J-map profile, '+time[ii].cashew_time,$
              yrange=[-3,max(col)+1],/nodata,charsize=2,xrange=minmax(lat),/xstyle
        oplot,lat,col,color=40
        oplot,lat,colSmooth,color=70,thick=4
        oplot,lat,sgsmooth,color=220,thick=4
        oplot,minmax(lat),[data_background,data_background],linestyle=2,color=150,thick=4
...
;Plot the wave profile
     ...
     if ii gt sp then begin
        ...
        !x=profile_x
        !y=profile_y
        ;latrange=[lastpeaklat-15.,lastpeaklat+15.]
        ;if latrange[0] lt 0. then latrange[0]=lat[0]
        ;if latrange[1] gt lat[n_elements(lat)-1] then latrange[1]=lat[n_elements(lat)-1]
        ;intensityrange=[-3.,wave_backedge[ii-1].val+1]
        latrange=minmax(lat)
        intensityrange=[-3,max(col)+1]
     endif else begin
        !p.position=profile_position
        latrange=minmax(lat)
        intensityrange=[-3,max(col)+1]
     endelse
     ;latindrange=get_index_range(lat,latrange)
     ;lintensityindrange=get_index_range(col,intensityrange)    
     
     !p.multi=profile_multi
     plot, lat, col, xtitle='Lateral distance, deg',ytitle='Pixel intensity',$
           charsize=2,/ystyle,/xstyle,$
           title=strtrim(string(ii),2)+' '+lat_data.type+' J-map profile, '+time[ii].cashew_time,/nodata,$
           yrange=intensityrange,xrange=latrange
     oplot,lat,col,color=40
     oplot,lat,colSmooth,color=70,thick=4
     oplot,lat,sgsmooth,color=220,thick=4
     oplot,latrange,[data_background,data_background],linestyle=2,color=150,thick=4
     xyouts,!p.position[2]-0.16,0.87,'Orig. data',color=0,/norm,charsize=2
     xyouts,!p.position[2]-0.16,0.84,'smooth(data)',color=70,/norm,charsize=2
     xyouts,!p.position[2]-0.16,0.81,'savgol(data)',color=220,/norm,charsize=2
     ...
     if ii gt sp and started_measurements eq 1 then begin
        ...
        oplot, [lastpeaklat,lastpeaklat], !y.crange,color=50,thick=2
        oplot, [lastpeaklat,lastpeaklat], !y.crange,color=180,thick=2,linestyle=2
...
;Plot the wave J-map with the limits and the current time step.
     ...
     aia_plot_jmap_data,time.jd,yarray[yrng[0]:yrng[1]],$
                        data[*,yrng[0]:yrng[1]],$
...
;------------------- END OVERVIEW PLOT ----------------------
     ...
     if uinput eq 'n' then begin
        if started_measurements eq 0 then lat_data.timefitrange[0]=lat_data.timefitrange[0]+1
        if started_measurements eq 1 then begin 
           lat_data.timefitrange[1]=ii-1
           if (timefitrange[0] ne lat_data.timefitrange[0]) or $
              (timefitrange[1] ne lat_data.timefitrange[1]) then begin
              print,''
              ...
              if uinput eq 'y' then begin
                 ...
                 printf,lun,strtrim(string(lat_data.timefitrange[0]),2)
                 printf,lun,strtrim(string(lat_data.timefitrange[1]),2)
                 close,lun
                 print,''
     ...
     tmp=min(where(lat gt x))
     if tmp[0] eq -1 then tmp = n_elements(lat)-1
     backEdge=lat[tmp]
     wave_backedge[ii].lat = backEdge        ;the lateral position
     wave_backedge[ii].yind = tmp            ;the lateral position index
     ...
     if tmp[0] eq -1 then tmp = n_elements(lat)-1
     peaklat=lat[tmp]
     wave_peak[ii].lat = peaklat         ;the lateral position
     wave_peak[ii].yind = tmp            ;the lateral position index
     wave_peak[ii].xind = ii             ;the time index
     wave_peak[ii].val = diffcol[tmp]    ;the intensity value
     oplot, [peaklat,peaklat], !y.crange,color=0,thick=3
     oplot, [peaklat,peaklat], !y.crange,color=150,thick=3,linestyle=2
     ...
     if tmp[0] eq -1 then tmp = n_elements(lat)-1
     frontEdge=lat[tmp]
     wave_frontedge[ii].lat = frontEdge       ;the lateral position
     wave_frontedge[ii].yind = tmp            ;the lateral position index
...
;------------------- START OVERVIEW PLOT ----------------------     
     ...
     !p.multi=overview_multi
     ;aia_plot_jmap_data,time.jd,lat_data.yarray[yrng[0]:yrng[1]],$
     ;                   data[*,yrng[0]:yrng[1]],$
     ...
     loadct,39,/silent
     plots,time[ii].jd+dt2,wave_frontedge[ii].lat,psym=sym(3),symsize=2,color=130
     plots,time[ii].jd+dt2,wave_frontedge[ii].lat,psym=sym(8),symsize=2,color=0,thick=3
     plots,time[ii].jd+dt2,wave_peak[ii].lat,psym=sym(1),symsize=2,color=130
     plots,time[ii].jd+dt2,wave_peak[ii].lat,psym=sym(6),symsize=2,color=0,thick=3
     plots,time[ii].jd+dt2,wave_backedge[ii].lat,psym=sym(2),symsize=2,color=130
     plots,time[ii].jd+dt2,wave_backedge[ii].lat,psym=sym(7),symsize=2,color=0,thick=3
;------------------- END OVERVIEW PLOT ----------------------
...
;Wave average intensity over time
  ...
  for tt=xrng[0],xrng[1] do $
     ...
     then lat_data.avgintense[tt]=mean(lat_data.diffdata[tt,backinds[tt]:frontinds[tt]])
!p.multi=0
end

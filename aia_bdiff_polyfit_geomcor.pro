...
;accelerations. This version performs geometric correction of the positions.
;
;CATEGORY:
; AIA/Kinematics
;
...
jdfrac[i]=tmp.frac
if i eq init then print, tmp
endfor

time=jdfrac[init:fin]

print,indices[init].date_obs
...
;Do geometric_correction
geomcorpeakpos=peakpos
geomcorfactor=1.0/sin(arlon*!PI/180.0)
ntimes=n_elements(peakpos[0,0,*,0])
ntrials=n_elements(peakpos[0,*,0,0])
nprofs=n_elements(peakpos[*,0,0,0])
...
;First, get mean and standard deviation of the positions
   for i=0,fin-init do begin
      res=moment(sqrt((geomcorpeakpos[p,*,i,0]-proforigin[0])^2+(geomcorpeakpos[p,*,i,1]-proforigin[1])^2),sdev=sdev)
      sigmapos[p,i]=sdev
      cmemoments[p,i,0]=reform(res[0])
      cmemoments3d[p,i,0]=reform(res[0])
   ;  sigmapos[p,i]=stddev(sqrt(geomcorpeakpos[p,*,i,0]^2+geomcorpeakpos[p,*,i,1]^2))
   ;  cmemoments[p,i,0]=average(sqrt((geomcorpeakpos[p,*,i,0]-proforigin[0])^2+(geomcorpeakpos[p,*,i,1]-proforigin[1])^2))
   endfor

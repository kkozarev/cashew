Pro	display_yafta, image, $ ; renamed version of DISPLAY.PRO, 03 May 2007
		xs, ys, $
...
;	2006: B.T. Welsch, added some plot formatting commands	
;	2009 Apr 01: BTW, renamed this procedure, appending "_yafta"
;	2009 Apr 02: BTW, renamed "imgexp.pro" and "imgscl_fen.pro",
;            appending "_yafta"; removed CAPS in calls to these, for grep
;-
...
;
	...
	psis = 512.0
    EndIf
    im = imgexp_yafta(image, xs, ys, xscale, yscale, xrange, yrange, $
                      Aspect=aspect, Interpolate=Keyword_Set(interp), $
                      MaskValue=maskvalue, Position=dev_pos, $
                      PS_Interp_Size=psis, $
...
;
    If ( sz(sz(0)+1) GT 1 ) Then Begin
        byte_im = imgscl_fen_yafta(im, Min=minval, Max=maxval, $
                                   Top=!D.Table_Size-2, $
                                   Log=log_scaling, Levels=l, $
                                   MaskValue=maskvalue)
    EndIf Else Begin

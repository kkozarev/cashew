...
pro aia_compare_shock_pfss_0613,test=test ;subindex,subdata
...
;for arien
        if pcname eq 'arien' then begin
           ...
           print,'restoring '+datapath+'pfss_results_20110511_1.1Rs_dens_1.sav'
           restore,datapath+'pfss_results_20110511_1.1Rs_dens_1.sav'
...
;Also, rescale the PFSS to the solar radius in pixels
  pfss_sphtocart,ptr,ptth,ptph,l,b,pxt,pzt,pyt


;--------------------------------------------------------------
print,'restoring '+datapath+'AIA_20110511_37_193_shocklocations.sav'
restore,datapath+'AIA_20110511_37_193_shocklocations.sav'
...
;LOOP OVER ALL AIA TIME STEPS HERE
  ...
  ;ntimes=1
  for t=0.0D,ntimes*1.0D-1 do begin

;Get the Carrington longitude and latitude of SDO
  l=subindex[t].crln_obs*!PI/180.0
  b=subindex[t].crlt_obs*!PI/180.0

  px=pxt*subindex[t].r_sun
  py=pyt*subindex[t].r_sun
  pz=pzt*subindex[t].r_sun

  nlines=n_elements(px[0,*])*1.0D
  maxnpts=n_elements(px[*,0])*1.0D
...
;create rotation and translation matrix
     pfss_cartpos=fltarr(nlines*1.0D,3,maxnpts)
     ff=0.0D
     for ff=0.0D,nlines*1.0D-1 do begin
...
;Alternatively, I will have to substitute yafta by clicking on points...
     ;radius=500
     rsun_km=6.955e5
     radius=RADIUSFITLINES[t]/rsun_km*kmpx
...
;This will have been previously determined and added to the index.
     ...
     lon=84.0
     arcoords=[lat,lon]
     aia_plot_hemisphere,subindex[t],radius,arcoords=arcoords,vertex_list=vertex_list

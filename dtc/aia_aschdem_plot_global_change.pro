...
;________________________DISPLAY EM-BASED GLOBAL CHANGE________________________
  ...
  if keyword_set(density) then begin
     aia_aschdem_calculate_em,event,basefname[0],em_map,density=basemap
  endif else begin
     if keyword_set(temperature) then begin
        aia_aschdem_calculate_em,event,basefname[0],em_map,temperature=basemap
     endif else begin
  ...
  if keyword_set(density) then begin
     ...
     aia_aschdem_calculate_em,event,infname,em_map,density=inmap
     ...
     if keyword_set(temperature) then begin
        ...
        aia_aschdem_calculate_em,event,infname,em_map,temperature=inmap
        if keyword_set(bratio) then begin
           plotim=inmap/(1.*basemap+1.e-30)
           dmin= 1.0
           dmax= 1.6
        ...
        aia_aschdem_calculate_em,event,infname,inmap
        if keyword_set(bratio) then begin

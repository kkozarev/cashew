...
pro create_event_folders,event
 ;PURPOSE:
...
;Written by Kamen Kozarev, 01/2012
  ...
  subfolders={radio:['NRH','IPS'],annulusplot:['araw','abase','arun'],png:['raw','base','run'],dem:['aschwanden','weber']}
  
 ; if keyword_set(label) then event=load_events_info(label)
  if size(event,/type) ne 8 then return
  if not dir_exist(event.savepath) then spawn,'mkdir '+event.savepath


  for f=0,n_elements(folders)-1 do begin
     folder=folders[f]
     if not dir_exist(event.savepath+folder) then spawn,'mkdir '+event.savepath+folder
     
     ;make the radio subfolders
     if folder eq 'radio' then $
        for i=0,n_elements(subfolders.radio)-1 do $
           if not dir_exist(event.savepath+folder+'/'+subfolders.radio[i]) then $
              spawn,'mkdir '+event.savepath+folder+'/'+subfolders.radio[i]
     
     ;make the radio subfolders
     if folder eq 'annulusplot' then $
        for i=0,n_elements(subfolders.annulusplot)-1 do $
           if not dir_exist(event.savepath+folder+'/'+subfolders.annulusplot[i]) then $
              spawn,'mkdir '+event.savepath+folder+'/'+subfolders.annulusplot[i]

     ;make the png subfolders
     if folder eq 'png' then $
        for i=0,n_elements(subfolders.png)-1 do $
           if not dir_exist(event.savepath+folder+'/'+subfolders.png[i]) then $
              spawn,'mkdir '+event.savepath+folder+'/'+subfolders.png[i]
     
     ;make the dem subfolders
     if folder eq 'dem' then $
        for i=0,n_elements(subfolders.dem)-1 do $
           if not dir_exist(event.savepath+folders[f]+'/'+subfolders.dem[i]) then $
              spawn,'mkdir '+event.savepath+folders[f]+'/'+subfolders.dem[i]
  endfor

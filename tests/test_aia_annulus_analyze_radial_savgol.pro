;+============================================================================
pro test_aia_annulus_analyze_radial_savgol
...
;You can run for one event, like this.
  ...
  if one eq 1 then begin
     ...
     aia_annulus_analyze_radial_savgol,event,wave=wav,/constrain ;,rrange=rrange ;,/interactive
  endif
...
;n_elements(events)-1
     for ev=0,n_elements(events)-1 do begin
        ...
        for w=0,n_elements(wavelengths)-1 do begin
           ...
           aia_annulus_analyze_radial_savgol,event,wave=wavelength,rrange=rrange,/constrain ;,/interactive
        endfor
     endfor
  endif
...
;+============================================================================
function jmap_filter_maxima_radial_savgol,time,height,mymaxima,fitrange=fitrange
;PURPOSE:
...
;Perform Savitzky-Golay filter fitting (no bootstrapping yet)
  sgfpix=4
  order=0
  sgfil=savgol(sgfpix,sgfpix,order,4)
  savgol_xfit=convol(dist,sgfil,/edge_truncate)
; plot,time_good,dist,/ynozero,psym=2
; oplot,time_good,convol(dist,sgfil,/edge_truncate),linestyle=2
  order=1
  sgfil_v = SAVGOL(sgfpix, sgfpix, order, 4)*(FACTORIAL(order)/(dt^order))
  savgol_vfit=convol(dist,sgfil_v,/edge_truncate)
; plot,time_good,deriv(time_good,dist),/ynozero,psym=2
; oplot,time_good,convol(dist,sgfil_v,/edge_truncate),linestyle=2 
  order=2
  sgfil_a = SAVGOL(sgfpix, sgfpix, order, 4)*(FACTORIAL(order)/(dt^order))
  savgol_afit=convol(dist,sgfil_a,/edge_truncate)
...
; oplot,time_good,convol(dist,sgfil_a,/edge_truncate),linestyle=2
  datastruct.maxkin[sp:ep].x=savgol_xfit/(DIST_FACTOR*height)
  datastruct.maxkin[sp:ep].v=savgol_vfit
  datastruct.maxkin[sp:ep].a=savgol_afit
...
;+============================================================================
pro aia_annulus_analyze_radial_savgol,event,datapath=datapath,savepath=savepath,thrange=thrange,$
...
;Create the radial data structure
  ...
  rad_data={$
           ...
           maxkin:replicate({x:0.0,v:0.0,a:0.0},nsteps),$
...
;DESPIKE THE IMAGE
  ...
  annulus_fit_maxima_radial_savgol,event,rad_data.bdiff,rad_data,time,y_rsun_array,constrain=constrain, auto=auto, gradient=gradient
